	...
	while (true) {
		for (int i = 0; i < velocX.size(); i++) {
			NavierStokesEquations(
				...
				[&velocX](Mat<>& _x) { return velocX(min(max((int)_x[0], 0), N - 1), min(max((int)_x[1], 0), M - 1)); },
				[&velocY](Mat<>& _x) { return velocY(min(max((int)_x[0], 0), N - 1), min(max((int)_x[1], 0), M - 1)); },
				[](Mat<>& x) { return 0.0; },
				[&P](Mat<>& _x) { return P(min(max((int)_x[0], 0), N - 1), min(max((int)_x[1], 0), M - 1)); },
